...
bool eval (string result_sha) {
  ...
  int sequence_id = std::stoi(result_sha);
  // file name
  char file_name[256];
  sprintf(file_name,"%02d.txt", sequence_id);
  char pose_name[256];
  snprintf(pose_name, 256, "pose_%02d.txt", sequence_id);
  char trajectory_name[256];
  snprintf(trajectory_name, 256, "trajectory_%02d.txt", sequence_id);
  
  // read ground truth and result poses
  vector<Matrix> poses_gt     = loadPoses(result_dir + "/data/" + pose_name);
  vector<Matrix> poses_result = loadPoses(result_dir + "/data/" + trajectory_name);
 
  // plot status
  printf("Processing: %s, poses: %d/%d\n",pose_name,poses_result.size(),poses_gt.size());
  ...
  if (sequence_id<=15) {
    ...
    plotPathPlot(plot_path_dir,roi, sequence_id);

    // save + plot individual errors
    char prefix[16];
    sprintf(prefix,"%02d", sequence_id);
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  std::cout << "result_sha = " << result_sha << std::endl;
